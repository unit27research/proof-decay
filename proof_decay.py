...
class ProofNote:
    ...
    def summary(self, review_date: date) -> str:
        return (
            f"{self.artifact or self.label}; date: {self.artifact_date.isoformat()}; "
            f"age: {self.age_days(review_date)} days; scope: {self.scope_supported or 'not stated'}"
        )
        ...
        reviews.append(
            {
                ...
                "artifact_age": f"{proof.age_days(current_date)} days" if proof else "unknown",
...
def classify_decay(proof: Optional[ProofNote], review_date: date) -> List[str]:
    ...
    flags: List[str] = []
    if proof.age_days(review_date) > proof.freshness_window_days:
...
def suggest_bounded_wording(claim: str, proof: Optional[ProofNote], decay_flags: List[str]) -> str:
    ...
    if proof:
        ...
        artifact_date = proof.artifact_date.isoformat()
...
def _structured_note(fields: Dict[str, str]) -> ProofNote:
    ...
    return ProofNote(
        ...
        artifact_date=_parse_date(fields.get("artifact_date", "1970-01-01")),
